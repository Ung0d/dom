...
namespace dom
{
    ...
    */
    ...
    private:
        ...
        ChunkedArray<EntityData> mEntityData;
    ...
    template<typename C, typename ...PARAM>
    ComponentHandle EntityManager<CINDEX, COMP_TOTAL>::assignComponent(PARAM&&... param)
    {
    ...
    template<typename CINDEX, CINDEX COMP_TOTAL>
    template<typename C>
    C& EntityManager<CINDEX, COMP_TOTAL>::getComponent(ComponentHandle h)
    {
        ChunkedArray<C>* ca = static_cast<ChunkedArray<C>*>( mManagers[ ComponentTraits<C, CINDEX, COMP_TOTAL>::getID() ].get() );
        return ca->get(h);
    }
    ...
    template<typename CINDEX, CINDEX COMP_TOTAL>
    template<typename C>
    const C& EntityManager<CINDEX, COMP_TOTAL>::getComponent(ComponentHandle h) const
    {
        ChunkedArray<C>* ca = static_cast<ChunkedArray<C>*>( mManagers[ ComponentTraits<C, CINDEX, COMP_TOTAL>::getID() ].get() );
        return ca->get(h);
    }
    ...
    template<typename CINDEX, CINDEX COMP_TOTAL>
    void EntityManager<CINDEX, COMP_TOTAL>::destroy(CINDEX cid, ComponentHandle h)
    {
